...
class FlowNet2C(FlowNetC.FlowNetC):
    ...
    def forward(self, inputs):
        ...
        flow4_up = self.pad(flow4_up, out_conv3_1.size())
        out_deconv3 = self.pad(out_deconv3, out_conv3_1.size())
        concat3 = torch.cat((out_conv3_1,out_deconv3,flow4_up),1)

        flow3       = self.predict_flow3(concat3)
        flow3_up    = self.upsampled_flow3_to_2(flow3)
        out_deconv2 = self.deconv2(concat3)
        flow3_up = self.pad(flow3_up, out_conv2a.size())
        out_deconv2 = self.pad(out_deconv2, out_conv2a.size())
